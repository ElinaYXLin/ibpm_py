"""Driver: run py/ibpm.py on the low_re/ClarkY and low_re/GM15 airfoils,
same methodology as run_all_airfoils.py (SD7003/SD8000, now in high_re/):
...
SURF_test/low_re/README.md for the mentor question this answers (does a
...
Usage:  python3 SURF_test/run_low_re_airfoils.py
Output: SURF_test/low_re/{ClarkY,GM15}/_run_data/, SURF_test/batch_results_low_re.json
"""
...
CASES = {
    "ClarkY": dict(
        dat=REPO / "SURF_test" / "low_re" / "ClarkY" / "clarky.dat.txt",
        Re=60700,
        polar_alphas=[-2.07, 1.02, 4.16, 7.19, 10.26],
        conv_alpha=-0.45,
    ),
    "GM15": dict(
        dat=REPO / "SURF_test" / "low_re" / "GM15" / "gm15.dat.txt",
        Re=40600,
...
def main():
    ...
    results_path = REPO / "SURF_test" / "batch_results_low_re.json"
    ...
    for name, cfg in CASES.items():
        ...
        for alpha in cfg["polar_alphas"]:
            if alpha in done_alphas:
                continue
            t0 = time.time()
            outdir = REPO / "SURF_test" / "low_re" / name / "_run_data" / f"polar_a{alpha:+.2f}"
        ...
        for lvl in CONV_LEVELS:
            ...
            outdir = REPO / "SURF_test" / "low_re" / name / "_run_data" / f"conv_{lvl['tag']}"
            fpath, elapsed = run_case(
