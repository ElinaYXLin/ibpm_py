"""C++ counterpart of run_low_re_airfoils.py: runs build/ibpm on the same
low_re/ClarkY and low_re/GM15 polar sweep + grid convergence cases (same
...
already generated by run_low_re_airfoils.py's make_raw_for_dx.

Usage: python3 SURF_test/run_low_re_airfoils_cpp.py
Output: SURF_test/low_re/{ClarkY,GM15}/_run_data_cpp/, SURF_test/batch_results_low_re_cpp.json
"""
import json
import pathlib
import subprocess
...
def geom_path_for_dx(name, dx):
    ...
    if not p.exists():
        raise FileNotFoundError(
            f"{p} missing -- run SURF_test/run_low_re_airfoils.py first "
...
def main():
    ...
    results_path = REPO / "SURF_test" / "batch_results_low_re_cpp.json"
    ...
    for name, cfg in CASES.items():
        ...
        for alpha in cfg["polar_alphas"]:
            if alpha in done_alphas:
                ...
                continue
            outdir = REPO / "SURF_test" / "low_re" / name / "_run_data_cpp" / f"polar_a{alpha:+.2f}"
        ...
        for lvl in CONV_LEVELS:
            ...
            outdir = REPO / "SURF_test" / "low_re" / name / "_run_data_cpp" / f"conv_{lvl['tag']}"
            fpath, elapsed = run_case_cpp(
