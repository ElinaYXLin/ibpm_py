"""C++ counterpart of run_low_re_flowfield.py: runs build/ibpm on the
same low_re/ClarkY and low_re/GM15 flowfield case (same geometry, alpha,
grid, dt, nsteps, restart cadence) so gen_low_re_flowfield_figs.py can
show a Python-vs-C++ vorticity-field comparison.

Usage: python3 SURF_test/run_low_re_flowfield_cpp.py
Output: SURF_test/low_re/{ClarkY,GM15}/_run_data_cpp/flowfield/
"""
import pathlib
import subprocess
import sys
...
def main():
    ...
    for name, cfg in CASES.items():
        outdir = REPO / "SURF_test" / "low_re" / name / "_run_data_cpp" / "flowfield"
