...
its own question (see `../GM15/` for the "lower Re, same-ish airfoil
complexity" question instead).

## `1-orig/`

The only content generated so far for this airfoil (mirrors what
`high_re/SD7003/2-c++included/` shows, but there's no earlier
...
fine grid and for the `ngrid=3` investigation in `high_re/SD7003/3-ngrid=3/`.
...
mechanism `high_re/SD7003/README.md` documents for its own dx=0.01 case,
