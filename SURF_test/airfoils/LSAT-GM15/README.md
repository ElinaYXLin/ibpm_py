...
`../LSAT-ClarkY/` is). Picked as the "hard" case of this pair: more strongly
nonlinear/cambered Cl-alpha behavior than Clark-Y at this Re (see
`1-orig/polar_comparison.png`).

## `1-orig/`

Same content/style as `../LSAT-ClarkY/1-orig/` and `airfoils/`'s
...
airfoil's sweep (unlike `../LSAT-ClarkY/`'s coarse-grid case), despite the
lower Re.
