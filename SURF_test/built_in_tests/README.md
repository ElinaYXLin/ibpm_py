...
| File | What it is |
|---|---|
| `generate_validation_report.py` | The script that produces everything below. Re-run it with `python3 SURF_test/built_in_tests/generate_validation_report.py` (requires a local C++ build at `build/ibpm`; see the top-level `README`). Takes ~2 minutes total for all three cases. |
...
python3 SURF_test/built_in_tests/generate_validation_report.py
```
