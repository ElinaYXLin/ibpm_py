...
def main():
    ...
    def draw_field(ax, field, title, pts, alpha_deg):
        # NOTE(fix): py/ibpm.py's `-alpha` only tilts the free-stream
        # (BaseFlow); it is never applied to the geometry, so `field` is in
        # the frame where the body sits at its raw, UNROTATED orientation.
        # This used to rotate only the drawn outline by -alpha_deg without
        # rotating `field` to match -- inconsistent, and equivalent to
        # plotting the body about one grid cell away from where it truly
        # sits (at this dx, alpha). Plotting the raw points directly
        # matches what was actually solved. `alpha_deg` is kept for
        # call-site compatibility but no longer used for a rotation.
