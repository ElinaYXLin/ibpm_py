...
- **Case**: cylinder (diameter 1), Re=100, RK3, single-domain
  ...
  resolution, same reasoning as documented in `results/SD7003/README.md`
  (mismatched spacing makes the projection matrix singular).
- **Grid sizes**: `nx=ny` in {100, 200, 300, 400}. `dt=0.01` at
  `nx=400` (else `0.02`) -- `results/README.md` already documented
...
| `run_ibpm_case.py` | Tiny wrapper working around this environment's `py`-package-name shadowing issue (see `results/vortall/README.md`) so `py/ibpm.py` can be invoked as a plain subprocess. |
...
(`results/README.md` covers the same finding at one resolution; this
...
finding in `results/README.md`. ms/step numbers (`tables/cost_summary.md`):
...
airfoil studies in `results/SD7003/`, `results/SD8000/` were kept to
